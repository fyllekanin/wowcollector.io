...
function openModal() {
  if (!petViewBuilderStore.hasChanges) {
...
function openNamePromptModal() {
  modal.open(SubmitViewModal, {
    onConfirm: async (state: EmitState) => {
      try {
        const viewId = await $fetch('/api/item-view/pet', {
          method: 'POST',
          body: JSON.stringify({
            name: state.name,
            categories: petViewBuilderStore.getFinalCategories,
      ...
      <UDashboardNavbar class="" :ui="{ wrapper: 'pb-' }">
        <template #title>
          <div class="flex gap-2 items-center">
            ...
            <h2 class="text-lg font-semibold">Pet View Builder</h2>
          </div>
        </template>
        ...
        <template #right>
          <UButton
            :disabled="
              !petViewBuilderStore.hasChanges || !petViewBuilderStore.isValid
            "
            icon="mdi:plus"
